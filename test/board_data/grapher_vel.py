...
def make_test_graph():
    ...
    title = ""
    x_axis = ""
    fig.add_trace(go.Scatter(x=df['time'], y=df['vel'], mode='lines',name='Magik'))

    fig.add_trace(go.Scatter(x=df_vega['ts'], y=df_vega['velocity'],
               mode='lines',
               name='Cats Vega'))
    fig.add_trace(go.Scatter(x=df_vega['ts'], y=df_vega['eggVel'], mode='lines', connectgaps=True, name='Eggtimer'))
    x_axis = "Speed (m/s)"
    title = "Velocity comparison"
